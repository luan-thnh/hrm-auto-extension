...
/**
 * Load .cs.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .cs.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.cs.json')
      : path.join(process.cwd(), '.claude', '.cs.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .cs.json config
