/**
 * Shared utilities for ClaudeSkill hooks
 *
...
const LOCAL_CONFIG_PATH = '.opencode/.cs.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.cs.json');
 ...
 * @param {Object} config - ClaudeSkill config
 ...
 *   2. Global config (~/.opencode/.cs.json) - user preferences
 *   3. Local config (./.opencode/.cs.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.CK_DEBUG) {
      console.error(`[cs-config] Warning: ${validation.error}`);
    }
  }
