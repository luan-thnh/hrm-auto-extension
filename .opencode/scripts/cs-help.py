...
Usage:
    python cs-help.py                    # Overview with quick start
    python cs-help.py fix                # Category guide with workflow
    python cs-help.py plan:fast          # Command details
    python cs-help.py debug login error  # Task recommendations
    python cs-help.py auth               # Search (unknown word)
"""
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ck.json", ".cs.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .cs.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "ClaudeKit Configuration (.cs.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.opencode/.cs.json`"),
            ("Local", "Override per-project in `./.opencode/.cs.json`"),
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    help_cmd = f"/{prefix}cs-help" if prefix else "/cs-help"
...
def show_config_guide() -> None:
    """Display comprehensive .cs.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# ClaudeKit Configuration (.cs.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.opencode/.cs.json` (user preferences)")
    print("- Local: `./.opencode/.cs.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.opencode/.cs.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.opencode/.cs.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.opencode/.cs.json - applies everywhere")
    ...
    print("# Local (.opencode/.cs.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.opencode/.cs.json` - personal preference")
    print("- Local: `./.opencode/.cs.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.cs.json`")
...
def main():
    ...
    if input_str.lower() in ["config", "configuration", ".cs.json", "ck.json"]:
        show_config_guide()
        return
