---
description: "ClaudeSkill usage guide - just type naturally - Args: ['category|command|task description']"
---

Think harder.
All-in-one ClaudeSkill guide. Run the script and present output based on type markers.
...
```bash
python .opencode/scripts/cs-help.py "$ARGUMENTS"
```
...
- Use global (~/.opencode/.cs.json) for personal preferences like language, issue prefix style
- Use local (./.opencode/.cs.json) for project-specific paths, naming conventions
