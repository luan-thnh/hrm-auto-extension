---
description: ClaudeSkill usage guide - just type naturally
...
```bash
python .claude/scripts/cs-help.py "$ARGUMENTS"
```
...
- Use global (~/.claude/.cs.json) for personal preferences like language, issue prefix style
- Use local (./.claude/.cs.json) for project-specific paths, naming conventions
