#!/usr/bin/env node
/**
 * Tests for cs-config-utils.cjs - Config and path utilities
 * Run: node --test .claude/hooks/__tests__/cs-config-utils.test.cjs
 *
 * Covers:
...
const {
  ...
  getGitRoot,
  DEFAULT_CONFIG
} = require('../lib/cs-config-utils.cjs');

describe('cs-config-utils.cjs', () => {
