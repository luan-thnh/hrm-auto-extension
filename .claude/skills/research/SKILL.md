...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.cs.json` (or `~/.claude/.cs.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.claude/.cs.json`: `gemini.model` (default: `gemini-3.0-flash`)
