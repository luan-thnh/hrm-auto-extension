...
const MONOREPO_DIR = '/home/kai/claudeskill';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'claudeskill-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from claudeskill-engineer submodule
  const submodulePath = '/home/kai/claudeskill/claudeskill-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'claudeskill-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'claudeskill-cli', 'Should find claudeskill-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'claudeskill' matches multiple projects
  const result = run('create claudeskill test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
